...
public class Ex8Page2 implements View {
    ...
    public Component getViewComponent() {
        ...
        final Label page = new Label("page2");

        prev.addClickListener((Button.ClickListener) event -> UI.getCurrent().getNavigator().navigateTo("ex8page1"));

        next.addClickListener((Button.ClickListener) event -> UI.getCurrent().getNavigator().navigateTo("ex8page3"));

        verticalLayout.addComponent(counter);
        verticalLayout.addComponent(new HorizontalLayout(prev, next));
